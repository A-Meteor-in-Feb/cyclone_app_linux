...
void run_subscriber_application(int vehicle_id) {
	...
	dds::topic::Topic<VehicleData::steeringWheel> steeringWheel_topic(participant, "steeringWheel_topic");
	//dds::topic::Topic<VehicleData::joyStick_data> joyStick_topic(participant, "joyStick_topic");

	dds::sub::DataReader<VehicleData::steeringWheel> steeringWheel_reader(vehicle_subscriber, steeringWheel_topic);
	//dds::sub::DataReader<VehicleData::joyStick_data> joyStick_reader(vehicle_subscriber, joyStick_topic);

	dds::sub::LoanedSamples<VehicleData::steeringWheel> sw_samples;
	...
	while (!shutdown_requested) {
		...
		if (sw_samples.length() > 0) {
			...
			dds::sub::LoanedSamples<VehicleData::steeringWheel>::const_iterator iter;
			for (iter = sw_samples.begin(); iter < sw_samples.end(); ++iter) {

				const VehicleData::steeringWheel& data = iter->data();
				const dds::sub::SampleInfo& info = iter->info();

				std::cout << "steeringWheel data: " << data << std::endl;
